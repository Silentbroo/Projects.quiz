...
int main() {
	...
	ALLEGRO_FONT *font = al_load_ttf_font("luv.ttf", 70, 0);
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (bouncer_y < 0 || bouncer_y > SCREEN_H - BOUNCER_SIZE) {
				...
				bouncer2_x = 250 + .25*(cos(t) + t*sin(t));
				bouncer2_y = 250 + (.25*(sin(t) - t*cos(t)))*-1;
			...
			if (redraw && al_is_event_queue_empty(event_queue)) {
				...
				////
				al_clear_to_color(al_map_rgb(200 + t, t, 50 + t));
				al_set_target_bitmap(al_get_backbuffer(display));
				al_draw_bitmap(bouncer, bouncer_x, bouncer_y, 0);
				al_draw_bitmap(bouncer, bouncer_x + 450, bouncer_y, 0);
				al_draw_bitmap(bouncer, bouncer_x + 200, bouncer_y + 450, 0);
				////
				al_draw_bitmap(bouncer2, bouncer2_x, bouncer2_y, 0);
				al_draw_bitmap(bouncer2, bouncer2_x+450, bouncer2_y, 0);
				al_draw_bitmap(bouncer2, bouncer2_x+200, bouncer2_y+450, 0);

				al_draw_text(font, al_map_rgb(255, 255, 255), 700 / 2, (1600 / 4), ALLEGRO_ALIGN_CENTRE, "You make my stomach ");
				al_draw_text(font, al_map_rgb(255, 255, 255),1400 / 2, (2000 / 4), ALLEGRO_ALIGN_CENTRE, " go in swirls!");
				al_flip_display();
